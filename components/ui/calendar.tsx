...
function CalendarDayButton({
  ...
  // Custom styling for booked and disabled dates
  ...
  return (
    <Button
      ...
      className={cn(
        'data-[selected-single=true]:bg-[#591FFF] data-[selected-single=true]:text-white data-[range-middle=true]:bg-accent data-[range-middle=true]:text-accent-foreground data-[range-start=true]:bg-[#591FFF] data-[range-start=true]:text-white data-[range-end=true]:bg-[#591FFF] data-[range-end=true]:text-white group-data-[focused=true]/day:border-ring group-data-[focused=true]/day:ring-ring/50 dark:hover:text-accent-foreground flex aspect-square size-auto w-full min-w-8 flex-col gap-1 leading-none font-normal group-data-[focused=true]/day:relative group-data-[focused=true]/day:z-10 group-data-[focused=true]/day:ring-[3px] data-[range-end=true]:rounded-md data-[range-end=true]:rounded-r-md data-[range-middle=true]:rounded-none data-[range-start=true]:rounded-md data-[range-start=true]:rounded-l-md [&>span]:text-xs [&>span]:opacity-70',
        // Booked dates styling - red background
        isBooked && 'bg-red-100 text-red-600 font-bold border-2 border-red-300 cursor-not-allowed hover:bg-red-100 hover:text-red-600',
        // Disabled dates styling (past, today, weekends) - gray background
        isDisabled && !isBooked && 'bg-gray-100 text-gray-400 cursor-not-allowed hover:bg-gray-100 font-semibold',
        // Available dates - normal hover effect
        !isDisabled && !isBooked && 'hover:bg-[#591FFF]/10 cursor-pointer rounded-xl',
        defaultClassNames.day,
        className,
      )}
      {...props}
